...
public class PanelSnakePM extends PanelSnake {

    private Thread hilo;
    private AccionPM accionPM;

    /**
     * Metodo constructor de la clase PanelSnakeP1
     */
    public PanelSnakePM(Color cuerpo, Color cabeza, int tamMax, int numCuadros, SnOOPe snoope, SnOOPeGUI gui) {
        super(cuerpo, cabeza, tamMax, numCuadros, snoope, gui);
        this.snoope.setColores(cuerpo, 1);
        cuerpoSnake = snoope.getSnakeP3Body();

        accionPM = new AccionPM(this, gui);
        hilo = new Thread(accionPM);
        hilo.start();
    }
    ...
    public int veolicidadSerpiente() {
        return snoope.getSnakeP3().getVelocidad();
    }
        ...
        try {
            for (int[] tuple : cuerpoSnake) {
                if (Arrays.equals(tuple, cuerpoSnake.get(cuerpoSnake.size() - 1))) {
                    pintor.setColor(snoope.getSnakeP3().getColorCabeza());
                } else {
                    pintor.setColor(snoope.getSnakeP3().getColorCuerpo());
                }
